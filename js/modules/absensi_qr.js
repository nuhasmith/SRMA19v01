// ============================================================
//  ABSENSI_QR.JS – Versi Final Lengkap (Admin)
//  SRMA 19 Bantul
//  Fitur: Info Sesi, Mode Manual, Custom Sesi, Form Tambahan, Log
// ============================================================
...
(function() {
                ...
                <div class="mt-2 pt-2 border-top">
                    ...
                    <div id="manualSessionSelector" style="display:none;">
                        <div class="d-flex flex-wrap gap-2 align-items-center">
                            <select class="form-select form-select-sm rounded-pill flex-grow-1" id="manualSessionSelect" onchange="onManualSessionChange()">
    ...
    function suggestSessionByTime() {
        if (isManualMode && !manualSesi) {
            ...
            sesiList.forEach(s => {
                ...
                const end = h2 * 60 + m2;
                if (currentMinutes >= start && currentMinutes <= end) {
                    const diff = currentMinutes - start;
                    if (diff < bestDiff) {
                        bestDiff = diff;
                        bestMatch = s;
                    }
            ...
            if (bestMatch) {
                const select = document.getElementById('manualSessionSelect');
                if (select) {
                    select.value = bestMatch.kode;
                    onManualSessionChange();
                    window.showToast('⏰ Sesi "' + bestMatch.nama + '" sedang berlangsung, otomatis dipilih.', 'info');
                }
            } else {
                window.showToast('Tidak ada sesi yang sedang berlangsung saat ini.', 'info');
            }
        }
    }
    ...
    window.onManualSessionChange = onManualSessionChange;
    window.showCustomSessionForm = showCustomSessionForm;
    window.cancelCustomSession = cancelCustomSession;
    window.applyCustomSession = applyCustomSession;
    window.confirmAbsensi = confirmAbsensi;
    window.resetScan = resetScan;
    window.clearLocalLog = clearLocalLog;

    console.log('✅ Absensi QR module loaded (admin)');
})();